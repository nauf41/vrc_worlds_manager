...
pub async fn main(app: AppHandle) -> anyhow::Result<()> {
    ...
    loop {
        ...
        while let Some((begin, entry)) = process_target.next() {
            ...
            if crate::db::log_files::get_log(entry.file_name().to_str().unwrap())
                .await?
                .is_none()
            {
                crate::db::log_files::new_log(entry.file_name().to_str().unwrap()).await?;
            }
            crate::db::log_files::update_log_read_at(
                entry
                    .file_name()
                    .to_str()
                    .ok_or(anyhow::anyhow!("Log file name missing"))?,
                len,
            )
            .await
            .unwrap();
...
fn process_file<R: std::io::BufRead + std::io::Seek>(
    mut reader: R,
) -> anyhow::Result<(i64, Vec<Session>)> {
    ...
    loop {
        ...
        if header_regex.is_match(&buf) {
            ...
            let time = chrono::Utc
                .with_ymd_and_hms(years, months, days, hours, minutes, seconds)
                .unwrap();
        ...
        if enter_room_uuid_regex.is_match(&buf) {
            ...
            session_from = Some(
                now_header
                    .ok_or(anyhow::anyhow!("Missing log header"))?
                    .time,
            );
...
mod tests {
    ...
    impl Into<String> for LogBuilder {
        fn into(self) -> String {
            ...
            format!(
                ...
                self.header.time.month(),
                self.header.time.day(),
                self.header.time.hour(),
                self.header.time.minute(),
                self.header.time.second(),
                self.header.level.into_str(),
                self.body
            )
        }
    }
    ...
    fn get_bufreader_from_string(s: &str) -> BufReader<Cursor<String>> {
        BufReader::new(Cursor::new(s.to_owned()))
    }
    ...
    fn check_single_session() {
        let log: Vec<(LogHeader, &str)> = vec![
            ...
            (
                ...
                "[Behaviour] Joining wrld_1234567890:00000",
            ),
            (
                LogHeader::new(
        ...
        let log: Vec<String> = log
            .iter()
            .map(|p| LogBuilder::new(p.0, p.1.to_owned()).into())
            .collect();
    ...
    fn check_multiple_sessions() {
        let log: Vec<(LogHeader, &str)> = vec![
            ...
            (
                ...
                "[Behaviour] Joining wrld_2234567890:00000",
            ),
            (
                LogHeader::new(
