...
pub async fn get_worlds(
    filter: worlds::WorldQueryFilters,
    sort_by: worlds::SortBy,
) -> Option<Vec<worlds::World>> {
...
pub async fn upsert_tag_group_attachment(tagid: i64, taggroupid: Option<i64>) -> bool {
    tag_groups::upsert_attachment(tagid, taggroupid)
        .await
        .is_ok()
}
...
pub async fn get_discord_channels(
    guild_id: String,
) -> Option<Vec<(Option<String>, Vec<ChannelInfo>)>> {
    if let Ok(v) = crate::discord_bot::http::get_channels(serenity::all::GuildId::new(
...
pub async fn add_discord_link(
    ...
    if let Err(_) =
        crate::db::discord::upsert_channel(channel.id.parse().unwrap(), channel.name).await
    {
        return false;
    }
    if let Err(_) = crate::db::discord::create_link(
        tag_id,
...
pub async fn parse_channel(
    ...
    crate::discord_bot::http::get_worlds_from_channel(channel, offset.parse().unwrap())
        .await
        .ok()
        .and_then(|v| Some((v.0.and_then(|v| Some(v.to_string())), v.1)))
}
