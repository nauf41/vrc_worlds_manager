...
impl serenity::client::EventHandler for Handler {
  async fn message(&self, _ctx: Context, msg: serenity::model::channel::Message) {
    ...
    log::info!("Received message in channel {}: {}", msg.channel_id, msg.content);
    // check match
    for wl in &mut wl {
      let tmp: i64 = msg.channel_id.get().try_into().unwrap();
      if wl.discord_channel_id == tmp {
        log::info!("Matched channel {} with tag_id {}", wl.discord_channel_id, wl.tag_id);
        let mut wrld: Vec<WorldDBStructure> = vec![];
        process_message(wl.latest_read_id.unwrap_or(0).try_into().unwrap(), &msg, &mut wrld).await;
        if wrld.len() > 0 {
          crate::db::tags::attach(wl.tag_id, wrld[0].id, true).await.unwrap();
        }
        crate::db::discord::upsert_channel(
          wl.discord_channel_id,
          msg.channel_id.to_string(),
        ).await.unwrap();
        crate::db::discord::update_link(wl.tag_id, wl.discord_channel_id, msg.id.get() as i64).await.unwrap();
