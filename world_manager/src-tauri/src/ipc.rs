...
pub async fn main(app: AppHandle) -> anyhow::Result<()> {
    let receiver = tokio::spawn(async move {
        let server = ServerOptions::new()
            ...
            .create(r"\\.\pipe\io.github.nauf41.world_manager.tauri")
            .unwrap();
        ...
        loop {
            ...
            if len > 64 * 1024 * 1024 {
                // Since there's 64 MiB outbound limitation on Chrome Native Messaging API
...
pub async fn process_and_gen_response(
    app: &AppHandle,
    msg: native_messaging::Message,
) -> anyhow::Result<native_messaging::Response> {
    use native_messaging::{CheckFavoriteResponse, MessageBody, Response, ResponseBody};
    use tauri::Emitter;

    match msg.body {
        MessageBody::FavoriteStatus(dat) => {
            let uuid = dat.uuid;
            let is_favorite = crate::db::worlds::does_tag_contain_world(
                0,
                crate::db::worlds::get_id_from_uuid(&uuid)
                    .await?
                    .unwrap_or(-1),
            )
            .await?;
            ...
            Ok(Response {
                id: msg.id,
                body: ResponseBody::FavoriteStatus(CheckFavoriteResponse { uuid, is_favorite }),
            })
        }

        MessageBody::UpdateCache(cache) => {
            crate::db::worlds::upsert_world(cache.world).await?;
        ...
        MessageBody::SetRegistered(req) => {
            ...
            log::info!(
                "Setting registered status for world {} to {}, id: {:?}",
                req.world,
                req.is_registered,
                world_id
            );
